  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (postStair.isNotEmpty) {
      ...
      for (var pt in postStair) {
        canvas.drawLine(
            Offset(pt[0] + baluster, size.height - pt[1] + factor), Offset(pt[0] + 6 * factor, size.height - pt[1] - postHeight + landingHeight), postPen);
    ...
    if (postFlatLower.isNotEmpty) {
      if (data['hasBottomCrotchPost']) {
        canvas.drawLine(
            Offset(firstEscalonX - factor - bottomCrotchLength, postFlatLower.first[1]), Offset(postFlatLower.first[0], postFlatLower.first[1]), postPen);
      } else {
